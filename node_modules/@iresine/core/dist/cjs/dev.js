...
var objectPath = require('@iresine/object-path');
var helpers = require('@iresine/helpers');
...
class Iresine {
  ...
  join(storeId) {
    const model = this.models.get(storeId);
    const templateObj = objectPath__default['default'].joinTemplate(model.template);
    ...
    for (let [path, storeId] of model.refs) {
      ...
      objectPath__default['default'].set(templateObj, path, templateObjChild);
    }
  ...
  joinRefs(template, refs) {
    ...
    const base = objectPath__default['default'].joinTemplate(template);

    for (const [path, modelId] of refs.entries()) {
      objectPath__default['default'].set(base, path, this.get(modelId));
    }
  ...
  _insert(storeId, rawTemplate, parentModelIds) {
    ...
    if (oldChildren) {
      const childrenToRemove = helpers.setUniq(oldChildren, model.children);
    ...
    if (parentModelIds) {
      helpers.setAdd(model.parents, parentModelIds);
    }
  }
...
exports.Iresine = Iresine;
exports.default = Iresine;
